...
this.bot.onText(/\/tolov/, async (msg) => {
    ...
    this.bot.sendInvoice(chatId, "Mahsulot uchun to'lov", "Mahsulotlar uchun to'lov qilshingiz mumkin",
     "to'lov", this.clickToken, "UZS",
     [
        {
            label: "Mahsulot",
            amount: 1200000
...
///////////////////// add_food
        this.bot.onText(/\/mahsulot_qoshish/, async (msg) => {
          ...
          this.bot.on("message", async (msg) => {
            ...
            if (this.waitingForFoodPrice.get(chatId)) {
              ...
              return this.bot.sendMessage(
                ...
                "📂 Kategoriya tanlang:\n1️⃣ ichimliklar\n2️⃣ yeguliklar\n3️⃣ shirinliklar",
                {
                  reply_markup: {
                    keyboard: [
                        [{text: "ichimliklar"}, {text: "yeguliklar"}, {text: "shirinliklar"}]
                    ],
            ...
            if (this.waitingForFoodCategory.get(chatId)) {
              if (!["ichimliklar", "yeguliklar", "shirinliklar"].includes(text.toLowerCase())) {
                return this.bot.sendMessage(chatId, "❌ Noto‘g‘ri kategoriya! Iltimos, qayta urinib ko‘ring.");
              }
              ...
              this.bot.sendMessage(chatId, `✅ Yangi taom qo'shildi!\n📌 Nomi: ${this.foodName}\n💰 Narxi: ${this.foodPrice} so'm\n📂 Kategoriya: ${category}`);
            }
          });


          this.bot.onText(/\/(ichimliklar|yeguliklar|shirinliklar)/, async (msg, match) => {
            const chatId = msg.chat.id;
            
            if (!match || !match[1]) {
              return this.bot.sendMessage(chatId, "❌ Noto‘g‘ri buyruq! Iltimos, qayta urinib ko‘ring.");
            }
            ...
            if (foods.length === 0) {
              return this.bot.sendMessage(chatId, "❌ Hozircha bu kategoriyada mahsulot mavjud emas.");
            }
          ...
          ////////// Delete product
          this.bot.onText(/\/mahsulot_ochirish/, (msg) => this.handleDeleteProductCommand(msg));
        ...
        private async handleDeleteProductCommand(msg: TelegramBot.Message) {
          ...
          if (msg.from?.id !== this.ownerId) {
            return this.bot.sendMessage(chatId, "❌ Siz mahsulot o‘chirish huquqiga ega emassiz.");
          }
      
          this.waitingForProductDeletion.set(chatId, true);
          this.bot.sendMessage(chatId, "📝 O‘chirmoqchi bo‘lgan mahsulot nomini kiriting:");
        }
